Base.@kwdef mutable struct RABBIToutput 
    beam_number::Union{String,Missing} = missing
...
function read_outputs(path::String; filename::String ="run")
    ...
    for item in readdir(result_dir)
        if contains(item, "beam") && isdir(joinpath(result_dir, item))
            ...
            output = RABBIToutput()
            beam_number = match(r"beam\d+", beam_path).match
            output.beam_number = beam_number
    ...
    sorted_outputs = sort(outputs, by = x -> parse(Int, replace(x.beam_number, "beam" => "")))
